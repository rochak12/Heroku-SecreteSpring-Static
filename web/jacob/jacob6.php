...
<div class="w3-main" style="margin-left:250px">
  <div class="w3-row w3-padding-64">
     ...
     <div class="w3-content w3-container">
      ...
      <?php include ($_SERVER['DOCUMENT_ROOT']."/jacob/jacobChapters.php");?>
      ...
      <table>
<tr><td class="caption"> <span class="english"> jacob 6 </span><span class="nepali">/ याकूब 6</span></td></tr>
...
<?php
   ...
   $obj2 = json_decode($json2, true);
    ...
    $bookName = $obj['books'][2]['book'];
    $chapterName = $obj['books'][2]['chapters'][5]['chapter'];
    $GLOBALS['i'] = 0;
   
    /*foreach ($db->query("SELECT * FROM book where name_book ='$bookName'") as $booklist){
      $GLOBALS['bookNo'] =  $booklist['book_id'];
    }*/
   foreach ($obj['books'][2]['chapters'][5]['verses'] as $print )  
   {
     $flag1 = FALSE;
...
{       $username = ""; }
 ...
 {    $username = $_SESSION['username']; }
     ...
     /*if ($db->query("SELECT * FROM highlight where book_id=3 and chapter=6 and verse = $currentVerse and username = '$username' ")){
    foreach ($db->query("SELECT * FROM highlight where book_id=3 and chapter=6 and username = '$username' ") as $row){
     if ($row['verse'] === $print['verse']){
        ...
        echo $print['verse'] . " " ;
        echo $obj2['books'][2]['chapters'][5]['verses'][$i]['text'] . "</mark></td></tr>" ;
   ...
   echo $print['verse'] . " " ;
   echo $obj2['books'][2]['chapters'][5]['verses'][$i]['text'] . "</td></tr>" ;
