...
<div class="w3-main" style="margin-left:250px">
  <div class="w3-row w3-padding-64">
     ...
     <div class="w3-content w3-container">
      ...
      <?php include ($_SERVER['DOCUMENT_ROOT']."/4nephi/4nephiChapters.php");?>
      ...
      <table>
<tr><td class="caption"> <span class="english">4 Nephi 1 </span><span class="nepali">/ 4 नफी 1</span></td></tr>
...
<?php
   ...
   $obj2 = json_decode($json2, true);
    ...
    $bookName = $obj['books'][11]['book'];
    $chapterName = $obj['books'][11]['chapters'][0]['chapter'];
    $GLOBALS['i'] = 0;
   
    /*foreach ($db->query("SELECT * FROM book where name_book ='$bookName'") as $booklist){
      $GLOBALS['bookNo'] =  $booklist['book_id'];
    }*/
   foreach ($obj['books'][11]['chapters'][0]['verses'] as $print )  
   {
     $flag1 = FALSE;
...
{       $username = ""; }
 ...
 {    $username = $_SESSION['username']; }
     ...
     /*if ($db->query("SELECT * FROM highlight where book_id=11 and chapter=12 and verse = $currentVerse and username = '$username' ")){
    foreach ($db->query("SELECT * FROM highlight where book_id=11 and chapter=12 and username = '$username' ") as $row){
     if ($row['verse'] === $print['verse']){
        ...
        echo $print['verse'] . " " ;
        echo $obj2['books'][11]['chapters'][0]['verses'][$i]['text'] . "</mark></td></tr>" ;
   ...
   echo $print['verse'] . " " ;
   echo $obj2['books'][11]['chapters'][0]['verses'][$i]['text'] . "</td></tr>" ;
